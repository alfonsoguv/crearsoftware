...
const OUTPUT_REDIRECTS = path.join(ROOT, 'data', 'redirects-audit.txt');
...
async function main() {
  ...
  for (const p of posts) {
    ...
    const imagesDir = path.join(ROOT, 'images', 'blog');
    ...
    while ((match = imgRegex.exec(markdown)) !== null) {
      ...
      try {
        ...
        if (!originalUrl.includes('youtube.com') && !originalUrl.includes('vimeo.com')) {
          let ext = path.extname(urlObj.pathname).toLowerCase();
          if (!ext || !['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg'].includes(ext)) {
            ext = '.jpg'; // Fallback if no valid extension
          }
          const hash = crypto.createHash('md5').update(originalUrl).digest('hex').substring(0, 8);
          const sanitizedTitle = sanitizeFilename(p.slug || 'p').substring(0, 30);
          const localFilename = `${sanitizedTitle}-${hash}${ext}`;
          const localPath = path.join(imagesDir, localFilename);
          const relativeUrl = `/images/blog/${localFilename}`;
          downloads.push({ originalUrl, localPath, relativeUrl });
        }
      } catch (e) {}
    }
    ...
    for (const dl of downloads) {
      try {
        await downloadImage(dl.originalUrl, dl.localPath);
        markdown = markdown.split(dl.originalUrl).join(dl.relativeUrl);
      } catch (err) {
        // console.warn(`  Warning: Failed to download ${dl.originalUrl}: ${err.message}`);
      }
    }
  ...
  console.log('\nGenerating data/redirects-audit.txt...');
  const redirectLines = [
  ...
  console.log(`    data/redirects-audit.txt    ${redirectCount} rules`);
